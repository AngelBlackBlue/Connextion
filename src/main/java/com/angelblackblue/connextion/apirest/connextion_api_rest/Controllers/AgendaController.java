...
public class AgendaController {
    ...
    // @PatchMapping("/{id}")
    // public Agenda updateContact(@PathVariable UUID id, @RequestBody Agenda
    // updatedContact) {
    ...
    @PatchMapping("/{id}")
    public Agenda updateContact(@PathVariable UUID id, @RequestBody Map<String, Object> updates) throws Exception {
        Agenda contact = getContactById(id);

        for (Map.Entry<String, Object> entry : updates.entrySet()) {
            Field field = Agenda.class.getDeclaredField(entry.getKey());
            field.setAccessible(true);
            field.set(contact, entry.getValue());
        }
